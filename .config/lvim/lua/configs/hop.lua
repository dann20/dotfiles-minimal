...
local status_ok, hint = pcall(require, "hop.hint")
if not status_ok then
	return
end
...
vim.keymap.set(mode, ',l', function() hop.hint_words({ direction = hint.HintDirection.AFTER_CURSOR, current_line_only = true }) end)
vim.keymap.set(mode, ',h', function() hop.hint_words({ direction = hint.HintDirection.BEFORE_CURSOR, current_line_only = true }) end)
